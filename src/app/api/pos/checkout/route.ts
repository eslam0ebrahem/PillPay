...
import { withPermission, withFinancialRateLimit } from '@/lib/auth/middleware';
import { checkout } from '@/lib/services/pos.service';
import { withIdempotency } from '@/lib/utils/idempotency';

export const POST = withPermission(
    'pos.checkout',
    withFinancialRateLimit(
        withIdempotency(async (req: NextRequest, context) => {
            try {
                ...
                const status = message.includes('تم استهلاك المخزون بواسطة عملية أخرى')
                    ? 409
