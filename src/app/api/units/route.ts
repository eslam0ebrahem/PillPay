...
export const GET = withPermission('products.view', (req: NextRequest) =>
    apiHandler(async () => {
        await connectDB();
...
export const POST = withPermission('users.manage', (req: NextRequest) =>
    apiHandler(async () => {
        await connectDB();
        ...
        if (existing) {
            return NextResponse.json(
                { error: { message: 'Unit with this code already exists' } },
                { status: 400 }
            );
        }
